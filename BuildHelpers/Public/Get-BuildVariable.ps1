function Get-BuildVariable {
    ...
    $WeCanGit = (
        ...
        (Get-Command $GitPath -ErrorAction SilentlyContinue)
    )
    if ($WeCanGit) {
        $IGParams = @{
            Path    = $Path
    ...
    # Teamcity has limited ENV: values but dumps the build configuration in a properties file.
    $tcProperties = Get-TeamCityProperty
